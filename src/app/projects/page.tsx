    ...
    <div className="container mx-auto px-4 py-12 page-transition">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {projects.map((project, index) => {
          // Determine if we should show the image or the fallback
          const showImage = project.image && !imageError[project.id];
          ...
          return (
            ...
            >
              <div className={`relative h-48 w-full overflow-hidden ${!showImage ? `bg-gradient-to-br ${gradientClass}` : 'bg-gray-100'}`}>
                {showImage ? (
                  <Image
                    src={project.image!}
                    alt={project.title}
                  ...
                  <div className="flex items-center justify-center h-full w-full">
                    <div className="text-4xl text-white opacity-30 transform -rotate-12 group-hover:scale-110 transition-transform duration-500">
                      &lt;/&gt;
                    </div>
                  </div>
                )}
